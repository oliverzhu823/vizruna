...
export function SettingsDraftProvider({ children }: { children: ReactNode }) {
  ...
  const patch = useCallback((fn: (d: SettingsDraft) => SettingsDraft) => {
    setDraft((prev) => {
      ...
      window.dispatchEvent(new CustomEvent('pi-enterprise-desktop:asr-config-preview', { detail: next.asrConfig }))
      return next
    })
    notifySettingsDirtyChanged()
  }, [i18n])
  ...
  const discard = useCallback(async () => {
    ...
    previewDraftUi(d, i18n)
    setAsrConfigPreview(d.asrConfig)
    window.dispatchEvent(new CustomEvent('pi-enterprise-desktop:asr-config-preview', { detail: d.asrConfig }))
