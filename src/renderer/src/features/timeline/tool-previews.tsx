      ...
      <NativePreviewPanel itemRunId={item.runId}
        ...
        meta={
          <span className="text-[10px] text-green-600 dark:text-green-400">
            {i18n.t('timeline:toolPreview.write')}
          </span>
        }
        defaultOpen={false}
        flat={flat}
  ...
  return (
    <NativePreviewPanel itemRunId={item.runId}
      ...
      meta={
        <span className={cn('text-[10px]', item.isError ? 'text-destructive' : 'text-green-600 dark:text-green-400')}>
          {item.isError
            ? i18n.t('timeline:toolPreview.failed')
            : i18n.t('timeline:toolPreview.saved')}
        </span>
      }
      defaultOpen={false}
      flat={flat}
    >
      {output ? (
        ...
        <div className="p-2 text-[11px] text-foreground-secondary/60">
          {i18n.t('timeline:toolPreview.noDiff')}
        </div>
      )}
    </NativePreviewPanel>
  )
    ...
    <NativePreviewPanel itemRunId={item.runId}
      ...
      meta={
        <span className="text-[10px] tabular-nums text-foreground-secondary">
          {i18n.t('timeline:toolPreview.lines', { count: lineCount })}
        </span>
      }
      defaultOpen={false}
      flat={flat}
...
function GrepFindPreview({ item, isFind, flat }: { item: ToolTimelineItem; isFind?: boolean; flat?: boolean }) {
  ...
  return (
    <NativePreviewPanel itemRunId={item.runId}
      ...
      meta={
        <span className="text-[10px] tabular-nums text-foreground-secondary">
          {i18n.t('timeline:toolPreview.items', { count: lines.length })}
        </span>
      }
      defaultOpen={false}
      flat={flat}
    >
      {show.length === 0 ? (
        <div className="px-2.5 py-2 text-[11px] text-foreground-secondary">
          {i18n.t('timeline:toolPreview.noResults')}
        </div>
      ) : (
      ...
      {lines.length > PREVIEW && (
        ...
        >
          {expanded
            ? i18n.t('timeline:collapse')
            : i18n.t('timeline:toolPreview.expandAll', { count: lines.length })}
        </button>
      )}
    </NativePreviewPanel>
  )
...
function BashPreview({ item, flat }: { item: ToolTimelineItem; flat?: boolean }) {
  ...
  const exitHint = item.isError
    ? 'exit ≠ 0'
    : output
      ? i18n.t('timeline:toolPreview.completed')
      : ''
  ...
  return (
    ...
    >
      {output ? (
        ...
        <div className="px-2.5 py-2 text-[11px] text-foreground-secondary/70">
          {i18n.t('timeline:toolPreview.noOutput')}
        </div>
      )}
    </NativePreviewPanel>
  )
