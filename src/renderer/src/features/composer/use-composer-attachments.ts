...
export function useComposerAttachments(opts: {
  ...
  useEffect(() => {
    ...
    window.addEventListener('pi-enterprise-desktop:composer-attach-files', onAttach)
    return () => window.removeEventListener('pi-enterprise-desktop:composer-attach-files', onAttach)
  }, [insertMetas])
