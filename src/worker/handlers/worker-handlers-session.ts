...
export async function handleSetmodel(msg: WorkerIncomingMessage, reply: WorkerReply): Promise<void> {
  if (!st.session) {
    reply({ type: 'error', error: 'MODEL_SESSION_NOT_READY' })
    return
  }
  try {
    ...
    reply({ type: 'setModel-done', model })
  } catch (e: unknown) {
    const message = errorMessage(e)
    console.error('[Worker] setModel failed:', message)
    reply({ type: 'error', error: `setModel failed: ${message}` })
  }
}
