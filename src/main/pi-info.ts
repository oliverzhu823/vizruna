...
export function readPiInfo(): PiInfo {
  const agentDir = getPiAgentDirectory()
  const info: PiInfo = {
    sdkVersion: '',
    agentDir,
  ...
  try {
    ...
    if (existsSync(authPath)) {
      ...
      info.authProviders = Object.entries(auth as Record<string, Record<string, unknown>>).map(([provider, cred]) => {
        ...
        const configured =
          typeStr === 'api_key'
            ? (typeof (cred.key ?? cred.apiKey) === 'string' &&
                String(cred.key ?? cred.apiKey).length > 0) ||
              cred.apiKey != null
            : typeStr === 'oauth'
              ? (typeof cred.access === 'string' && cred.access.length > 0) ||
                cred.oauth != null
