...
app.whenReady().then(() => {
  ...
  void getManagedWorktreeService().reconcile().catch((error) => {
    console.warn('[managed-worktree] startup reconciliation failed:', error)
  })
  void getOrchestrationService().initialize().catch((error) => {
    console.warn('[orchestration] startup recovery failed:', error)
  })
...
async function gracefulShutdownWorkers(): Promise<void> {
  ...
  try {
    ...
    /* terminal may not have been initialized */
  }
  try {
