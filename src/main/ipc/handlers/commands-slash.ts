...
export function registerCommandsSlashHandlers(): void {
  registerHandler('ipc:commands.completions', async (req) => {
    const commandName = String(req.commandName || '').replace(/^\//, '').toLowerCase()
    if (commandName === 'login' || commandName === 'logout') {
      const prefix = String(req.argumentPrefix || '').toLowerCase()
      const providers = await getProviderAuthService().list()
      return {
        items: providers
          .filter((provider) =>
            commandName === 'logout'
          ...
          .filter(
            (provider) =>
              !prefix ||
              provider.providerId.toLowerCase().includes(prefix) ||
              provider.name.toLowerCase().includes(prefix),
          )
          .map((provider) => ({
