...
import { PRODUCT_CONFIG_STORE_NAME } from '@shared/product-identity'
...
type StoredProxyProfile = Omit<ProxyProfile, 'passwordConfigured'>
...
export interface StoreSchema {
  ...
  /** 语音输入 ASR 配置 */
  asrConfig: AsrConfig
  /** Provider-specific network routes. Never mutates system proxy settings. */
  providerRoutes: Record<string, Omit<ProviderRoute, 'provider'>>
  /** Proxy profile metadata only; passwords are encrypted by safeStorage. */
  proxyProfiles: StoredProxyProfile[]
}

const store = new Store<StoreSchema>({
  name: PRODUCT_CONFIG_STORE_NAME,
  defaults: {
    recentProjects: [],
    currentProject: null,
    windowBounds: null,
