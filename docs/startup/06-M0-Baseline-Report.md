...
固定快照的 `package.json` 声明 MIT；当时仓库没有独立 LICENSE 文件。2026-07-30，
上游通过提交 `0ae02be2e5e09586aa89c35358f1aab952705e6c` 补充标准 MIT
LICENSE，作者同时在 Issue #38 确认欢迎二次开发并要求清楚标注。

处理结论：

- 商业使用、修改和再分发的源码许可阻断已解除。
- 仓库保留 `NOTICE.md` 和上游 MIT 正文；安装包必须携带同一声明。
- 具体发布批次仍需复核依赖清单和 SBOM，但不再等待 pi-app 作者补充授权。
...
- [x] pi-app 商业使用与再分发许可获得上游 MIT LICENSE 和作者确认。
- [ ] Apple Developer ID 签名、公证和干净设备安装（M5 发布门禁）。
- [ ] 使用测试凭据完成一次真实 Provider 模型调用（需要受控凭据和网络环境）。

结论：M0 内部工程门禁通过，可以进入 M1。pi-app 源码许可证阻断已于
2026-07-30 解除；Apple 签名、公证和真实 Provider 调用继续作为正式客户版的发布门禁。
