...
Vizruna 是面向中国企业 AI 转型项目的本地 Agent 工作台。它以 `pi-app` 为桌面产品底座，保留其中文界面、文件工作台、会话树、扩展适配和独立 Worker 架构，重新实现 `pi-gui` 中适合企业工程场景的会话租约、Git Worktree、多 Agent 编排、集成终端和 Provider 登录能力。
...
| Q-10 | Pi SDK 版本策略 | 内嵌当前已测最新版 0.82.1；外部版本需满足版本与能力双门禁 | 已决 |
