...
2026-07-30 起，产品开发统一纳入 `Agent Studio → Agent 工厂 → 独立 Runtime` 路线。原 M0–M5 是 Studio 的桌面工程底座，不等于已经完成 Agent 工厂或客户 Runtime。完整产品边界见 `19-Studio-Factory-Runtime-Roadmap.md`。
...
| 产品阶段 | 目标 | 当前判断 | 退出结果 |
| --- | --- | --- | --- |
| S1 Agent Studio | 自己和合作伙伴稳定跑 Agent、做案例 | 进行中 | 高频真实使用，并持续形成 Agent Case |
| S2 Agent Factory | 案例可复测、版本化、评测和打包 | 未开始 | 可验证的 Agent Version 与 `.vizagent` 包 |
| S3 Independent Runtime | 把批准的成熟 Agent 交付给客户 | 未开始 | 客户无需 Studio 即可受控运行 Agent |

当前迭代重点是 S1 的 alpha.3：Agent Case 第一闭环。只有真实案例完成重复验证后，才进入 Agent Version 和 Runtime 设计。
...
以上日历时间成立的前提是至少两名工程师并行推进，并严格执行首发边界。pi-app
许可证结论已于 2026-07-30 由上游 MIT LICENSE 和作者确认解决；若 Pi SDK 接口或
签名资质阻塞，日历时间需要顺延。
